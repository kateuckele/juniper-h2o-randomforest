...
## IS Q MODELED IN THIS CODE?
Q <- h5read(h5_path, "Q")
q <- h5read(h5_path, "q")
## IS Q MODELED IN THIS CODE?
Q.mean <- apply(Q[, 2, ], 2, mean)

# substract ancestry from 1 so that pure western ancestry == 0
...
HybridQ <- data.frame(Q=Q_hyb, env_hyb, latlong_hyb)
...
## Quick missingness check (after drop_na this should be 0)
sum(is.na(Hybridq))

## Correlation snapshot for numeric predictors
num_df <- Hybridq %>% select(where(is.numeric))
cor_mat <- cor(num_df, use = "pairwise.complete.obs")
cor_mat[1(8, ncol(cor_mat)), 1(8, ncol(cor_mat))]

#INCLUDE OR NOT?
## Drop rows with missing values
Hybridq <- Hybridq %>% tidyr::drop_na()
dim(Hybridq)
```
